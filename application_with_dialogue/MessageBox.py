...
############################
#usage to get exe file with specific logo (MessageBox.ico):
# pyinstaller --onefile --windowed --noconsole --add-data "MessageBox.ico:." --icon=MessageBox.ico MessageBox.py
############################

def get_ini_file_path():
    # Get the directory of the currently running executable or script
    exe_dir = os.path.dirname(sys.executable if getattr(sys, 'frozen', False) else __file__)
    
    # Get the base name of the executable or script without the extension
    base_name = os.path.splitext(os.path.basename(sys.executable if getattr(sys, 'frozen', False) else __file__))[0]
    
    # Construct the expected .ini file path
    ini_file_path = os.path.join(exe_dir, f"{base_name}_params.ini")
...
def show_dialogue_from_ini(ini_file):
    ...
    dialog.iconbitmap(resource_path("MessageBox.ico"))
    ...
    ok_button.pack(pady=(0, 50), side='bottom')
...
import cProfile
cProfile.run(show_dialogue_from_ini(ini_file_path))
